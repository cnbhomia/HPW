...
Variables
    ...
    costroyal(years)     'cost from paying royalty each year for open mine'       
...
$gdxin datafiles.gdx
$load ifopen , ifoper, qmined, costroyal, totore
$gdxin
ifoper.l('M2','Y3')= 0;
Equations
    ...
    Royalties(y)   'Total royalty paid each year'
...
Royalties(y)..      costroyal(y) =E= sum(m, ifopen(m,y)* royalty(m)) ;

*discountrate(y)..   dSellPr(y) =E= SellPr* power[(1-drate),ord(y)-1] ;

ObjFunc..           profit =E= sum(y, totore(y)*dSellPr(y) - costroyal(y))  ;
