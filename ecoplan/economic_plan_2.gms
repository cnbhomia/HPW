...
Variables
    TargetCap         'Objective for maximizing productive capacity'
    ;
*year 0 values 
...
GoodsBalance(i,t)$dt(t)..     Output(i,t) + Store(i,t) =E= sum(j,InOutCoef(i,j)*x(j,t+1)) +
                                                    sum(j,UpCapCoef(i,j)*y(j,t+2)) + Demand(i) + Store(i,t+1);

horizonstore(i)..              Store(i,'6') =E= Store(i,'7');
...
objFunc1..           TargetCap =E= sum[(i), Capacity(i,'5')] ;

model ecoplan /all/;

solve ecoplan using LP maximizing TargetCap ; 
