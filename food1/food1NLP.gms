...
positive variables
    ...
    spec(month) 'spec of product each month' 
    ;
variable
...
*initialization and bounds on variables
spec.lo(month) =3; spec.up(month)=6;
RMpur.l(oil,month)= 100;
RMused.l(oil,month)=100;
RMstock.l(oil,month)=10;
product.l(month)=10;

equations
    stockcalc(oil,month) 'stock calculation'
    matbalance(month) 'material balance each month'
    specification(month) 'spec of final product'
...
specification(month).. sum(oil,RMused(oil,month)*hardness(oil)) - spec(month)*product(month) =e=0 ;
...
model food1 /all/;
option limrow=100
solve food1 using NLP maximizing profit;
