...
parameter processexists(process,month);
...
variables
    ...
    ;
*variable bounds
produced.up(p,month) = marketlim(p,month);
...
*    marketlimit(p,month) 'market limit on each product each month'
    objfunc
    ;
...
*marketlimit(p,month).. produced(p,month) =l= marketlim(p,month) ;



objfunc.. profit =e= sum((p,month),produced(p,month));
