...
Positive Variables
    ...
    Capital(yr)         'Capital Outlay /loan taken each year for extra cows[£/£200]'
...
herdsize.up('Y5') = 1.75 * 120 ;
herdsize.lo('Y5') = 0.5 * 120 ;
...
Equations
    ...
    CapOutlay(yr)           'calculating capital outlay for extra cattle'
...
populae1(cows,yr) ..               Pop('heifer',yr) =E= CowCount('0',yr) + CowCount('1',yr) ;

populae2(cows,yr) ..               Pop('milkcow',yr) =E= sum(a$[ord(a)>2 and ord(a)<13] ,CowCount(a,yr)) ;
...
growing(yr)..                      Ar4Sb(yr) + sum(g,Ar4Gr(g,yr)) =L= MaxArea;
...
OverPopulae(yr)..                  sum(cows,Pop(cows,yr)) =L= 130 + ExtraCows(yr)  ;

CapOutlay(yr)..                    capital(yr) =E= ExtraCows(yr) * CapOut ;

Income(yr)..                       Revenue(yr) =E= 370 * pop('milkcow',yr) + sum(f, fsold(f,yr)*SellPr(f))
                                                                    + 30 * bullsold(yr) + 40*heifsold(yr) +120 * oldcow(yr) ;

Expenditure(yr)..                  Expense(yr) =E= LaborCost(yr) + capital(yr) +  sum(f, fbuy(f,yr)*CostPr(f)) + OtherCost(yr);
